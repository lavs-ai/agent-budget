...
[!\[CI](https://github.com/lavs-ai/agent-budget/actions/workflows/ci.yml/badge.svg)](https://github.com/<you>/agent-budget/actions/workflows/ci.yml)
...
```bash
git clone https://github.com/lavs-ai/agent-budget.git
cd agent-budget
npm install
...
agent-budget check "..." --provider copilot\_agent\_mode
agent-budget status
...
agent-budget check "fix a typo in the README" --provider copilot\_agent\_mode
...
agent-budget check "Redesign the payment service for multi-region active-active, refactor across the whole repo" \\
  --provider copilot\_agent\_mode --files 12
...
1. Point `LEDGER\_PATH` at a shared SQLite file (Turso/LibSQL for zero-ops sync) or Postgres
